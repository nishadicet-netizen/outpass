...
public class TeacherViewOutpass extends AppCompatActivity implements JsonResponse, AdapterView.OnItemClickListener {


    ListView lv1;
    Button b1;
    String request,date,reason,duration;
    String[] dates,reasons,time,durations,pass_id,val,statuss,course_name,first_name,last_name;
    public static String pass_ids,statusss;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_teacher_view_outpass);
        ...
        JR.json_response=(JsonResponse) TeacherViewOutpass.this;
        String q = "/teacher_view_outpass_request_from_students";
        q=q.replace(" ","%20");
        JR.execute(q);
    }
    ...
    public void response(JSONObject jo) {
        ...
        try {
            ...
            if(method.equalsIgnoreCase("teacher_view_outpass_request_from_students")){
...
//            }
            ...
            if(method.equalsIgnoreCase("teacher_approve_outpass_request_from_students"))
            {
                String status=jo.getString("status");
                Toast.makeText(getApplicationContext(),status, Toast.LENGTH_LONG).show();
                if(status.equalsIgnoreCase("success"))
    ...
    public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
        ...
        if (statusss.equals("pending")) {
            ...
            AlertDialog.Builder builder = new AlertDialog.Builder(TeacherViewOutpass.this);
            ...
            builder.setItems(items, new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int item) {
                    if (items[item].equals("Accept")) {
                        JsonReq JR=new JsonReq();
                        JR.json_response=(JsonResponse) TeacherViewOutpass.this;
                        String q = "/teacher_approve_outpass_request_from_students?pass_id="+pass_ids;
                        q=q.replace(" ","%20");
                        JR.execute(q);
                        ...
                        JsonReq JR=new JsonReq();
                        JR.json_response=(JsonResponse) TeacherViewOutpass.this;
                        String q = "/teacher_reject_outpass_request_from_students?pass_id="+pass_ids;
                        q=q.replace(" ","%20");
                        JR.execute(q);
                    }
